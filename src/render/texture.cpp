...
Math::Vec3f Texture::sample(float u, float v) const {
    if(TextureMode::LINEAR == mode_) {  // 计算双线性插值
        ...
        // 边缘情况特殊处理
        if(u < 0 || v < 0) return image_.getColor<3>(x, y);
        if(x == image_.getHeight() - 1 || y == image_.getWidth() - 1) return image_.getColor<3>(x, y);
        
        Math::Vec3f c1 = image_.getColor<3>(x, y);
        Math::Vec3f c2 = image_.getColor<3>(x + 1, y);
        Math::Vec3f c3 = image_.getColor<3>(x, y + 1);
        Math::Vec3f c4 = image_.getColor<3>(x + 1, y + 1);
